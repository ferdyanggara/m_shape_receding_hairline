...
import cv2
import numpy as np 
from drawAndCalculate import draw_calc_brows
from print_utils import show, return_stats
from algorithm_utils import perform_top_operation, perform_bottom_operation

person = 1
def process(image_path, m_shape, verbose):
  global person
  ...
  for (i, rect) in enumerate(rects):
      ...
      for (name, lst) in FACIAL_LANDMARKS_IDXS.items():
        if name == 'left_eyebrow': 
          ...
          perform_top_operation(shape, lst, image, name, coordinates)
        elif name.startswith("bottom"):
          perform_bottom_operation(shape, lst, clone, name, coordinates, distance, iterList)
  '''
  detect m shape
  '''
  if m_shape == True:
